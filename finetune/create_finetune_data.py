...
# --- 1. CLASS NAMES (From 100_rgb.txt) ---
CLASS_NAMES = [
...
FINETUNE_SAVE_DIR = "finetune_data" # All new data will be saved here
...
# --- State Logic (Fixed Window) ---
STATE_WAITING = 0
STATE_RECORDING = 1
STATE_COOLDOWN = 2 # Removed processing state

TOTAL_FRAMES_FOR_MODEL = 29
FRAMES_TO_CAPTURE_PRE_TRIGGER = 9
FRAMES_TO_CAPTURE_POST_TRIGGER = 20
# Total frames = 9 + 20 = 29

# Set buffer size to be at least as large as pre-trigger frames
PRE_BUFFER_SIZE = 10 

# --- User-defined values ---
SAVE_COOLDOWN_S = 2.0 # Show "SAVED!" for 2 seconds
MOUTH_OPEN_THRESHOLD = 6.0 # Trigger threshold
# -------------------------------
...
def get_word_counts(save_dir, class_list):
    """Scans the save directory and counts existing clips for each word."""
...
def get_next_word(counts):
    """Finds the word with the minimum number of recordings."""
...
def main():
    ...
    # --- Scan for existing data and set start word ---
    ...
    recording_buffer = [] # Will store (frame)
    frames_to_collect = 0
    last_save_time = 0
    lip_dist = 0.0
    status_text = "READY"
        ...
        if len(faces) > 0:
            ...
            if roi:
                ...
                if current_state == STATE_WAITING:
                    status_text = "READY"
                    pre_buffer.append(resized_lip) # Buffer the 0-255 frame
                    ...
                    if lip_dist > MOUTH_OPEN_THRESHOLD:
                        ...
                        if len(pre_buffer) < FRAMES_TO_CAPTURE_PRE_TRIGGER:
                            ...
                            # Grab last N frames from pre-buffer
                    ...
                    if frames_to_collect == 0:
                        ...
                        # --- REMOVED PROCESSING, SAVING DIRECTLY ---
                        if len(recording_buffer) == TOTAL_FRAMES_FOR_MODEL:
                            ...
                            # --- Update counts and get next word ---
